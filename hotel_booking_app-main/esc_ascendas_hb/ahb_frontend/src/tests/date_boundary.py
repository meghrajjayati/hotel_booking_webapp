# 4 tests 
# 2 valid dates boundary and middle value 
# 2 invalid dates- boundary 1222 doesnt follow format, random string email
...
class autocompleteTest(unittest.TestCase):
    ...
    def test_exact_dest(self):
        ...
        randomDate= '0'+str(random.randint(1,9))+'/'+str(str(random.randint(23,28)))
        name.send_keys(("JP"))
        time.sleep(1)
        ...
        try:        
                booking_id= driver.find_element("xpath","/html/body/div/div/div/div/div/div[1]/div/div/div/div[2]")
                is_clickable = True
        except:
    ...
    def test_wrong_dest(self):
        ...
        it= random.randint(1,3)
        randomemail = ''.join(random.choice(string.ascii_letters) for _ in range(6))+"@gmail.com"
        randomDate= str(random.randint(10,12))+str(str(random.randint(23,28)))
        name.send_keys(("JP"))
        time.sleep(1)
        phone_number.send_keys((3883838383))
        time.sleep(1)
